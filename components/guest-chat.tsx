...
export function GuestChat({ initialMessage, onBack, onSignUp }: GuestChatProps) {
  ...
  // Save guest messages to localStorage for migration
  useEffect(() => {
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-xl font-bold text-foreground font-[var(--font-heading)]">AI Chat</h1>
            </div>
            <div className="flex items-center gap-4">
              <span className="text-sm text-muted-foreground">{promptCount}/2 Free Prompts Used</span>
              <Button onClick={onSignUp} size="sm" className="bg-white text-black hover:bg-gray-200">
          ...
          <div className="space-y-6 max-w-4xl mx-auto">
              ...
              {messages.map((message) => (
                ...
                >
                  {message.role === "assistant" && (
                     <div className="w-8 h-8 bg-transparent border border-white/10 rounded-full flex items-center justify-center flex-shrink-0">
                      <Bot className="w-4 h-4 text-foreground" />
                    </div>
                  )}

                  <Card
                    className={`max-w-[70%] border ${message.role === "user" ? "bg-white/10 text-white border-white/20" : "bg-transparent border-white/10 text-foreground"
                      }`}
                  >
                    <div className="p-4">
                      <p
                        className={`text-sm leading-relaxed whitespace-pre-wrap break-words font-[var(--font-body)] ${message.role === "user" ? "text-white" : "text-foreground"
                          }`}
                      >
                        {message.content}
                      </p>
                      <span
                         className={`text-xs mt-2 block ${message.role === "user" ? "text-white/70" : "text-muted-foreground"
                          }`}
                      >
            ...
            {isLoading && (
              ...
              >
                ...
                <Card className="bg-transparent border-white/10 text-foreground">
                  <div className="p-4">
              ...
              >
                <Card className="bg-white/5 border-white/10 max-w-md">
                  <div className="p-4 text-center">
                    <p className="text-sm text-foreground mb-3 font-[var(--font-body)]">
                      You've reached the free limit. Please log in to continue.
                    </p>
                    <Button onClick={onSignUp} className="bg-white text-black hover:bg-gray-200">
                      Login to continue
                    </Button>
                  </div>
                </Card>
            ...
            <div className="flex items-center gap-4">
              <div className="relative">
                <Button
                  ...
                  className="flex items-center gap-2 bg-background border-border hover:bg-white/5 hover:text-foreground"
                >
                ...
                {showTranslateDropdown && (
                  <div className="absolute bottom-full mb-2 w-48 bg-background border border-border rounded-lg shadow-lg z-20">
                    {Object.entries(languageConfig).map(([key, config]) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 hover:bg-white/5 transition-colors first:rounded-t-lg last:rounded-b-lg ${selectedLanguage === key ? "bg-white/10 text-foreground" : "text-muted-foreground"
                          }`}
                      >
                        {config.label}
                      </button>
              ...
              {selectedLanguage !== "english" && (
                <span className="text-sm text-muted-foreground">Selected: {languageConfig[selectedLanguage].label}</span>
              )}
            </div>
          </div>
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 relative">
                <Input
                  ...
                  className="pr-24 h-12 bg-transparent border-white/20 focus:border-white/50 font-[var(--font-body)] text-foreground placeholder:text-muted-foreground"
                />
